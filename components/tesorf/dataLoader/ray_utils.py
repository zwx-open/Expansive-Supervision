...
def get_sample_direction(X, Y, H, W, focal, center=None):
    X = X + 0.5 
    Y = Y + 0.5
    cent = center if center is not None else [W / 2, H / 2]
    ### Y → i; X → j;
    directions = torch.stack([(Y - cent[0]) / focal[0], (X - cent[1]) / focal[1], torch.ones_like(Y)], -1)  # (H, W, 3)
    return directions
...
def get_sample_direction_blender(X, Y, H, W, focal, center=None):
    X = X + 0.5 
    Y = Y + 0.5
    cent = center if center is not None else [W / 2, H / 2]
    directions = torch.stack([(Y - cent[0]) / focal[0], -(X - cent[1]) / focal[1], -torch.ones_like(Y)],
                             -1)  # (H, W, 3)
    return directions
...
def get_sample_rays(directions, c2w):
    """
    Inputs:
        directions: (numrays, 3) precomputed ray directions in camera coordinate
        c2w: (numrays, 4, 4) transformation matrix from camera coordinate to world coordinate
    Outputs:
        rays_o: (numrays, 3), the origin of the rays in world coordinate
        rays_d: (numrays, 3), the normalized direction of the rays in world coordinate
    """
    rays_d = torch.bmm(directions.unsqueeze(1), c2w[:, :3, :3].permute(0,2,1)) 
